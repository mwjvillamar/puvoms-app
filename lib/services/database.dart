...
class DatabaseService {
  ...
  Future updateQueueStatus(String uid, bool inQueue, DateTime queueStart,
      String firstName, String lastName, String plateNumber) async {
  ...
  Future addPassenger(String queueUID, int passengerCount) async {
    return await queueCollection
        .doc(queueUID)
        .update({'passengerCount': passengerCount});
  }

  Future createUser(String uid, String firstName, String lastName, String role,
  ...
  Future updateUser(
      String uid, String firstName, String lastName, String phoneNum) async {
  ...
  Future createVehicle(String uid, String vehicleBrand, String vehicleColor,
      String plateNumber, DateTime queueStart) async {
  ...
  Future updateVehicle(String uid, DateTime queueStart) async {
    return await vehicleCollection.doc(uid).update({'queueStart': queueStart});
  }
  ...
  Future createMessage(String senderUID, String senderName, String senderRole,
      String message, DateTime dateSent) async {
  ...
  List<MessageList> _messageListFromSnaphot(QuerySnapshot snapshot) {
    return snapshot.docs.map((doc) {
      return MessageList(
          ...
          dateSent: doc.get('dateSent').toDate());
    }).toList()
      ..sort((m1, m2) => m2.dateSent.compareTo(m1.dateSent));
  }
  ...
  List<PaymentCollection> _paymentsListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.docs.map((doc) {
      ...
      ..sort((p1, p2) => p1.datePaid.compareTo(p2.datePaid));
  }
  ...
  List<QueueCollection> _queueListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.docs.map((doc) {
      ...
      ..sort((q1, q2) => q1.queueStart.compareTo(q2.queueStart));
  }
  ...
  List<VehicleCollection> _vehicleListFromSnapshot(QuerySnapshot snapshot) {
    return snapshot.docs.map((doc) {
      ...
      ..sort((v1, v2) => v1.queueStart.compareTo(v2.queueStart));
  }
  ...
  Stream<List<PaymentCollection>> get paymentsList {
    return paymentCollection.snapshots().map(_paymentsListFromSnapshot);
  }

  Stream<List<MessageList>> get messageList {
    return messageCollection.snapshots().map(_messageListFromSnaphot);
  }
  ...
  Stream<List<PaymentCollection>> getDriverPaymentsList(String queueUID) {
    return paymentCollection
        .where("queueUID", isEqualTo: queueUID)
        .snapshots()
        .map(_paymentsListFromSnapshot);
  }
  ...
  Stream<QueueData> get queueData {
    return queueCollection.doc(uid).snapshots().map(_queueDataFromSnapshot);
  }

  //For Role Only?
  Stream<QueueData> get queueStatus {
    return queueCollection.doc(uid).snapshots().map(_statusData);
  }

  //Stream for Queuelist
  Stream<List<QueueCollection>> get queueList {
    return queueCollection.snapshots().map(_queueListFromSnapshot);
  }

  //Stream for Vehicle Data
  Stream<VehicleData> get vehicleData {
    return vehicleCollection.doc(uid).snapshots().map(_vehicleDataFromSnapshot);
  }
